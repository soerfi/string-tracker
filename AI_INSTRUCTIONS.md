...
- **Absolute Z-Index Stacking Bugs:** Popovers (like a CustomSelect dropdown) positioned `absolute z-50` will be heavily visually constrained if nested inside a `flex-1 overflow-y-auto` container. If the sibling is a `sticky top-0 z-10` header, the `overflow-y-auto` boundary forces the *entire content block* underneath the sticky header during scroll. Use native body-level layout scrolling (`min-h-[100dvh]` on outer wrapper without fixed `overflow`) to break elements out and over cleanly.
- **Intelligent Walk-In QR Loading:** `AdminForm.tsx` allows "Scannen" right from Step 1. Rather than blindly returning 404s for unregistered physical factory stickers, it queries `api/qr/validate` to verify if the string represents an Unused Database Code. If true, it automatically triggers the "New User/Racket" onboarding flow and secretly attaches that QR token to the final POST job payload!
