...
7. **Docker Phantom Containers**: When running `--no-cache` builds or wiping old images, older versions of `docker-compose` can crash with `KeyError: 'ContainerConfig'` because of orphaned hex-named shadow containers. Solve via `docker-compose down --remove-orphans && docker system prune -f`.
